...
\begin{figure}
\centering
\includegraphics[scale=.6, trim= 1cm 3cm 5cm 3cm, clip, width=1\columnwidth]{impl002.pdf}
...
\begin{figure}
\centering
\includegraphics[scale=.6, trim= 2cm 3cm 5cm 3cm, clip, width=1\columnwidth]{impl003.pdf}
\caption{Pure React force graph lifecycle visualization}
...
One of the most challenging aspects of the thesis project is the performance measurement of the prototypes. Modern broswers have an uncountable amount of features that help to smooth out the performance to improve user perception. Using different browsers for benchmarks also means, that different JavaScript runtimes are used to run the benchmarks.

When benchmarking web applications it is very complicated to get performance values that have scientific relevance in order to compare them to then get valid results. Subsection \ref{sub:perfImplDetails} introduces a system that was especially implemented to measure the performance of JavaScript web applications.

Another big proplem was the fact, that browsers will detect the refresh rate of monitors. When using monitors with a refresh rate of 144 hertz, frames per second can go up to 144 FPS. When using 60 hertz monitors though the framerate is limited to 60 FPS. The maximum amount of request animation frame calls in the browsers can go up to the hertz the browser window runs in. 

Speaking of the request animation frame functionality, it must be mentioned that each browser has a different implementation of the funtionality. For example chrome is smoothing out the performance by trying to fire the animation frames consistently, which means that the overall performance might be lowered in order to achieve a smoother framerate. Experience shows, that Firefox on the other hand always fires its animation frames whenever a frame is available which can result in higher overall but not as smooth framerates.

Last but not least another challenge was to build a performance measuring environement that can be used without adding code to the prototypes. Theoretically each prototype should be a complete component that can be shipped as a third party library. By adding code for performance measurement the components would contain functionality that is not needed when shipping production builds of the components.

\subsection{Building a stable Testing Environment}

To thest all three prototypes in a consistent way, a testing evironement is needed which produces consistent data across multiple instances. Of course when testing the force simulation prototypes the data they're tested with has to be consistent as well. Generating an arbitrary amount of random generated nodes and links is not a problem, as JavaScript has a random function which can be utilized to generate random data. Unfortunately the random generator cannot be seeded to get consistent pseudo random results. The library \emph{seedrandom} in \cite{SeedRandom} is the perfect technology to solve the problem of generating consistent data across multiple iterations of the benchmark tests.

The testing environment is basically designed to be a stateful container which generates some random data and then passes it to the desired prototype. As D3 simulations provide a mechanism to add an event handler whenever the simulaton stops, it is no problem to add a handler on the benchmark component that restarts the benchmark with some newly generated data until the desired amount of benchmark iterations is reached. 

Via a custom helper utility module it is possible to generate a specified amount of random nodes and links. The method to generate the random data takes exactly two parameters, the number of nodes and the number of links. To generate some consistent pseudo random force simulation data the previously mentioned seedrandom module is used internally.

Finally the benchmark tool has to be easy to use an all kinds of devices. Therefore a small React application was created which can be deployed on any static webhosting service. The application lets users select the desired force simulation implementation benchmark which will then run in the browser.

\subsection{Implementation details}
\label{sub:perfImplDetails}

I describe how I implemented the benchmark component.
