...
%% linear decrease but exponential difficulty???
...
Human vision is a very complicated topic, as there have been studies for years which tried to determine at which point humans percieve a series of images as fluid motion. Another crutial question is at which point an observer doesn't percieve an animation as fluid anymore. As the performance measurements provided definitive numbers, they can be used to determine if the animation of the test can be percieved as fluid motion or not. The answer of most research papers is though, that flickering or a so called "lag" cannot be detected anymore, if the human perception cannot distinguish anymore between modulated light and a stable field. The rate is between 50 and 90 hertz according to serveral articles like \cite{6375944}, \cite{farrell1987predicting} or \cite{stereoscopicFlickerArticle}.

Humans percive motion if the display produces more than 12 frames per second according to \cite{RestorationOfMotionPictureFilm}. Another article in \cite{Potter2014} suggests the same shit
